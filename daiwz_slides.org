...
\end{align}
{{{begin_div(,width:674px;font-size:1.19em)}}}
...
{{{begin_div(rightcol40)}}}
{{{rbold(符号表示)}}}
{{{begin_div(scalebox70 fragment fade-in,position:fixed;top:24.5%;left:55.5%,,1)}}}
$\mathbf{x}_i$: 样本{{{nl()}}}
$y_i$: 标记{{{nl()}}}
$h$: 假设模型{{{nl()}}}
$\mathcal{H}$: 假设空间{{{nl()}}}
$L$: 损失函数
{{{end_div()}}}
{{{begin_div(scalebox70 fragment fade-in,position:fixed;bottom:0%;left:51%,,3)}}}
$B$: 背景知识{{{nl()}}}
$E$: 样本{{{nl()}}}
$H$: 假设模型{{{nl()}}}
$\models$: 逻辑蕴涵
...
{{{end_div()}}}
{{{end_div()}}}
{{{begin_div(fragment fade-in,position:absolute;font-size:0.7em;right:0px;top:56%;,,3)}}}
#+BEGIN_SRC prolog :exports code
% 背景知识：
smokes(alan).
smokes(bob).
...
\[
\color{red}{w::}A\leftarrow B_1\wedge B_2\wedge\ldots\wedge B_n.
\]
{{{end_div}}}
#+ATTR_REVEAL: :frag (appear) :frag_idx (3)
- 学习目标: $\max_{(\Gamma,\theta)}P_\Gamma(E\mid\color{blue}{KB}\cup B,\theta)$
  #+ATTR_REVEAL: :frag (appear appear appear appear) :frag_idx (4 5 6 7)
  - 微调可能的规则结构 $\Gamma$;
  - 最大化样本似然，求解参数 $\theta$;
  - 若未达到终止条件, goto 2; 否则输出 $H=(\Gamma,\theta)$.  
{{{begin_div(popupbox fragment fade-in,position:absolute;top:40%;left:4%,,8)}}}
